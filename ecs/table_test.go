...
func TestNewTable(t *testing.T) {
	...
	assert.EqualValues(t, 16, table.entities.cap)
	for i := range table.columns {
		assert.EqualValues(t, 9, table.columns[i].len)
		assert.EqualValues(t, 16, table.columns[i].cap)
	}
	...
	assert.EqualValues(t, 16, table2.entities.cap)
	for i := range table2.columns {
		assert.EqualValues(t, 9, table2.columns[i].len)
		assert.EqualValues(t, 16, table2.columns[i].cap)
	}
}
