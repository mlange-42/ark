...
func TestToRelations(t *testing.T) {
	...
	out = inRelations.ToRelations(&w, all(ids...), ids, out[:0], nil)
	...
	expectPanicsWithValue(t, "component with ID 2 is not a relation component",
		func() {
			...
			_ = inRelations.ToRelations(&w, all(ids...), ids, out[:0], nil)
		})

	expectPanicsWithValue(t, "requested relation component with ID 0 was not specified in the filter or map",
		func() {
			ids := []ID{posID, childID, child2ID}
			_ = inRelations.ToRelations(&w, all(posID, child2ID), ids, out[:0], nil)
		})
	...
	out = inRelations.ToRelations(&w, all(ids...), ids, out, nil)
...
func BenchmarkToRelations0(b *testing.B) {
	...
	for b.Loop() {
		_ = rels.ToRelations(&world, &mask, ids, relations, nil)
	}
}
...
func BenchmarkToRelations1(b *testing.B) {
	...
	for b.Loop() {
		relations = rels.ToRelations(&world, &mask, ids, relations[:0], nil)
	}
}
...
func BenchmarkToRelations2Copy(b *testing.B) {
	...
	copyTo := make([]relationID, 4)

	for b.Loop() {
		relations = rels.ToRelations(&world, &mask, ids, relations[:0], copyTo[:0])
	}
}
